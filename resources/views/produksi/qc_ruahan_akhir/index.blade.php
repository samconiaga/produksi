    ...
    <div class="col-12">
      <div class="card">
        ...
        <div class="card-header d-flex justify-content-between align-items-center">
          <div>
            ...
            <p class="mb-0 text-muted">
              Konfirmasi <strong>Tanggal Datang Ruahan Akhir</strong>, mulai <strong>Analisa</strong> (Start),
              dan akhiri dengan <strong>Release</strong> (Stop). Pada tahap ini bisa sekaligus
              menandai batch sebagai <strong>QC RELEASED</strong> di proses utama.
            </p>
          </div>

          <a href="{{ route('qc-ruahan-akhir.history') }}"
             class="btn btn-sm btn-outline-secondary">
            Riwayat Produk Ruahan Akhir
          </a>
        </div>
        ...
        <div class="card-body border-bottom">
          <form method="GET" action="{{ route('qc-ruahan-akhir.index') }}" class="row g-1">
          ...
          <table class="table table-striped mb-0 align-middle">
            <thead>
              <tr>
                ...
                <th>WO Date</th>
                <th>Mixing</th>
                <th class="text-center">Tgl Datang Ruahan Akhir</th>
                <th class="text-center">Tgl Analisa Ruahan Akhir</th>
                <th class="text-center">Tgl Release Ruahan Akhir</th>
              </tr>
            </thead>

            <tbody>
            @forelse($batches as $idx => $batch)
              @php $formId = 'form-ruahan-akhir-' . $batch->id; @endphp
              ...
              <tr>
                ...
                <td class="text-center">
                  <div class="d-flex justify-content-center align-items-center gap-50">
                    <input type="date"
                           ...
                           form="{{ $formId }}"
                           value="{{ optional($batch->tgl_datang_ruahan_akhir)->format('Y-m-d') }}"
                ...
                <td class="text-center">
                  <div class="d-flex flex-column gap-25 align-items-center">
                    <span class="small text-muted">
                      {{ $batch->tgl_analisa_ruahan_akhir
                            ? $batch->tgl_analisa_ruahan_akhir->format('d-m-Y')
                            : '-' }}
                    </span>
                ...
                <td class="text-center">
                  <div class="d-flex flex-column gap-25 align-items-center">
                    <span class="small text-muted">
                      {{ $batch->tgl_rilis_ruahan_akhir
                            ? $batch->tgl_rilis_ruahan_akhir->format('d-m-Y')
                            : '-' }}
                    </span>
              ...
              <form id="{{ $formId }}"
                    action="{{ route('qc-ruahan-akhir.update', $batch) }}"
