    ...
    <div class="col-12">
      <div class="card">
        ...
        <div class="card-header d-flex justify-content-between align-items-center">
          <div>
            ...
            <p class="mb-0 text-muted">
              Konfirmasi <strong>Tanggal Datang Granul</strong>, mulai <strong>Analisa</strong> (Start),
              dan akhiri dengan <strong>Release</strong> (Stop). Data yang sudah release
              akan pindah ke menu <em>Riwayat Produk Antara Granul</em>.
            </p>
          </div>

          <a href="{{ route('qc-granul.history') }}"
        ...
        <div class="card-body border-bottom">
          <form method="GET" action="{{ route('qc-granul.index') }}" class="row g-1">
          ...
          <table class="table table-striped mb-0 align-middle">
            <thead>
              <tr>
                ...
                <th>WO Date</th>
                <th>Mixing</th>
                <th class="text-center">Tgl Datang Granul</th>
                <th class="text-center">Tgl Analisa Granul</th>
                <th class="text-center">Tgl Release Granul</th>
              </tr>
            </thead>

            <tbody>
            @forelse($batches as $idx => $batch)
              @php $formId = 'form-granul-' . $batch->id; @endphp
              ...
              <tr>
                ...
                <td class="text-center">
                  <div class="d-flex justify-content-center align-items-center gap-50">
                    <input type="date"
                           ...
                           form="{{ $formId }}"
                           value="{{ optional($batch->tgl_datang_granul)->format('Y-m-d') }}"
                ...
                <td class="text-center">
                  <div class="d-flex flex-column gap-25 align-items-center">
                    <span class="small text-muted">
                      {{ $batch->tgl_analisa_granul
                            ? $batch->tgl_analisa_granul->format('d-m-Y')
                            : '-' }}
                    </span>
                ...
                <td class="text-center">
                  <div class="d-flex flex-column gap-25 align-items-center">
                    <span class="small text-muted">
                      {{ $batch->tgl_rilis_granul
                            ? $batch->tgl_rilis_granul->format('d-m-Y')
                            : '-' }}
                    </span>
              ...
              <form id="{{ $formId }}"
                    action="{{ route('qc-granul.update', $batch) }}"
