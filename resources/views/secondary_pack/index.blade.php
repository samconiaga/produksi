    ...
    <div class="col-12">
      <div class="card">
          ...
          <div class="row align-items-center w-100">
            <div class="col-md-8">
              ...
              <p class="mb-0 text-muted">
                Proses <strong>Secondary Pack</strong> per batch dengan mode
                <strong>Start / Stop</strong> (realtime tanggal &amp; jam).
                Batch yang tampil adalah yang sudah selesai Primary Pack
                dan belum selesai Secondary Pack 1.
              </p>
            </div>
            <div class="col-md-4 text-md-end mt-2 mt-md-0">
              <a href="{{ route('secondary-pack.history') }}"
        ...
        <div class="card-body border-bottom">
          <form class="row g-2 align-items-center" method="GET" action="{{ route('secondary-pack.index') }}">
            <div class="col-md-4">
              <input type="text"
                     name="q"
                     value="{{ $q }}"
            ...
            <div class="col-md-2">
              <a href="{{ route('secondary-pack.index') }}"
          ...
          <table class="table mb-0 align-middle">
            <thead>
              <tr class="text-muted">
                ...
                <th width="160">Primary (Selesai)</th>
                <th width="180">Secondary (Mulai)</th>
                <th width="180">Secondary (Selesai)</th>
            ...
            @forelse($rows as $idx => $row)
              ...
              <tr>
                ...
                <td>
                  @if($row->tgl_secondary_pack_1)
                    {{ $row->tgl_secondary_pack_1->format('d-m-Y H:i') }}
                  @else
                    <span class="text-muted">-</span>
                  @endif
                </td>
                ...
                <td class="text-end">
                  <div class="d-flex justify-content-end align-items-center gap-50">
                    ...
                    @if(!$started && !$finished)
                      {{-- Belum mulai --}}
                      <form action="{{ route('secondary-pack.start', $row->id) }}" method="POST">
                        @csrf
                        <button type="submit"
                    ...
                    @elseif($started && !$finished)
                      ...
                      <form action="{{ route('secondary-pack.stop', $row->id) }}"
                            method="POST"
                            onsubmit="return confirm('Hentikan Secondary Pack dan lanjut ke Qty Batch untuk batch ini?');">
                        @csrf
                        <button type="submit"
                    ...
                    @else
                      {{-- Selesai: arahkan ke Qty --}}
                      <a href="{{ route('secondary-pack.qty.form', $row->id) }}"
