...
<section class="app-user">
  <div class="card">
    ...
    <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-1">
      <div>
        <h4 class="card-title mb-0">Riwayat Secondary Pack</h4>
        <small class="text-muted">
          Batch yang sudah selesai proses Secondary Pack (termasuk Rekon saat STOP).
        </small>
      </div>

      <a href="{{ route('secondary-pack.index') }}" class="btn btn-sm btn-outline-primary">
        Kembali ke Secondary Pack Aktif
      </a>
    </div>
    ...
    <div class="card-body">
      ...
      {{-- FILTER --}}
      <form class="row g-1 g-md-2 align-items-center mb-2" method="GET" action="{{ route('secondary-pack.history') }}">
        ...
        <table class="table table-sm table-hover align-middle">
          <thead class="table-light">
            <tr class="text-nowrap">
              ...
              <th>Secondary Selesai</th>
              <th class="text-end" style="width:140px;">Rekon</th>
          ...
          @forelse($rows as $idx => $row)
            @php
              $rekonQty = $row->secondary_pack_rekon_qty;
              $rekonAt  = $row->secondary_pack_rekon_at;
            ...
            <tr>
              ...
              <td>{{ optional($row->tgl_mulai_secondary_pack_1)->format('d-m-Y H:i') }}</td>
              <td>{{ optional($row->tgl_secondary_pack_1)->format('d-m-Y H:i') }}</td>

              <td class="text-end"><span class="fw-semibold">{{ $rekonText }}</span></td>
              <td>
                @if($rekonAt)
          ...
          @empty
            <tr>
              <td colspan="9" class="text-center text-muted">Belum ada riwayat Secondary Pack.</td>
            </tr>
          @endforelse
          </tbody>
        </table>
