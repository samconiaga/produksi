...
<style>
  ...
  .tablex thead th{font-size:.82rem;text-transform:uppercase;letter-spacing:.06em;color:#6b7280;background:#f9faffb;border-bottom-color:#e6e6ee;white-space:nowrap}
  ...
  <div class="col-12">
    <div class="card cardx">
      <div class="headx d-flex justify-content-between align-items-start flex-column flex-md-row gap-2">
        <div>
          <h4 class="titlex">SPV History (Processed)</h4>
          <div class="subx">Daftar dokumen SPV yang sudah diproses (APPROVED / REJECTED).</div>
        </div>

        <div class="d-flex gap-2">
          <a href="{{ route('spv.index') }}" class="btn btn-sm btn-outline-secondary">Back to Pending</a>
        </div>
      </div>

      <div class="bodyx">
        <form method="get" class="filter-wrap mb-3" action="{{ route('spv.history') }}">
